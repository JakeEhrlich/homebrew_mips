...
| Serial port | 10 x ATF22V10C (counted below) with an SP3232 RS-232 transceiver | **new**; a UART built from GALs as a bus device, docs/uart.md; the bus itself, docs/bus.md |
| Logic | 165 x ATF22V10C-7 | was 132; +2 write copies, +1 stall / output enable, +1 reset synchroniser, +9 boot copier, -1 forwarding control repack, +1 hold / bubble, +1 bus wait sequencer, +6 from the wait's hold inputs (forwarding control +2, operand B +1, branch target +1, stall +1, MEM/WB +1), -1 after the board-file completeness check found ten dead ID/EX outputs (the rs / rt numbers) and three dead branch-adder intermediates, +10 byte / halfword access (section 6.4), -8 when the I/O wait machinery was removed (the bus is fast-only, docs/bus.md), +10 the serial port, +1 the OEN / HOLD terms for I/O reads |

Total 186 chips plus the gate and the RS-232 transceiver.  The
...
This is also the machinery the narrow-store hold below reuses, and the
I/O read hold: a memory load right behind an I/O load holds one cycle,
because the SRAM's outputs are kept off for the cycle after a device
drove the bus (docs/bus.md section 4).
